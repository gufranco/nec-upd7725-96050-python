...
class AvailabilityTest(unittest.TestCase):
    ...
    def test_a_directory_that_is_not_there_offers_nothing_either(self):
        self.assertEqual(against_firmware.available(Path("/nowhere/at/all")), ())

    def test_the_reason_it_offers_nothing_names_where_it_looked(self):
        self.assertIn("firmware", WHY.lower())

    def test_and_names_the_setting_that_points_it_somewhere_else(self):
        self.assertIn("UPD7725_FIRMWARE_DIR", WHY)


class SummaryTest(unittest.TestCase):
    def test_a_comparison_that_found_nothing_says_nothing(self):
        self.assertEqual(against_firmware.summary({}), ())

    def test_and_one_that_found_something_says_how_much(self):
        found = against_firmware.summary({1: (), 2: ()})

        self.assertIn("2 of", found[0])

    def test_a_comparison_needs_both_masks_to_say_anything(self):
        self.assertEqual(against_firmware.compare_revisions(()), {})


class ReportTest(unittest.TestCase):
    def test_a_run_with_nothing_present_still_reports_rather_than_failing(self):
...
class BootTest(unittest.TestCase):
    ...
    def test_a_full_run_over_what_is_present_reports_clean(self):
...
class RevisionTest(unittest.TestCase):
    def test_the_two_masks_of_the_dsp1_do_not_answer_the_same_way(self):
