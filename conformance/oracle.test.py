...
def _sourced(block: reference.Words) -> oracle.Sourced:
    """The lazily-answered block a prepared reference carries.

    A prepared processor holds these rather than plain lists, and the started
    value is the thing only they know: what the seed put at an address before
    anything wrote to it.
    """
    assert isinstance(block, oracle.Sourced), "a prepared reference does not hold plain lists"
    return block


class PreparationTest(unittest.TestCase):
    def test_the_seed_decides_what_every_store_holds(self) -> None:
    ...
    def test_and_what_every_register_holds(self) -> None:
        chip = oracle.prepared(0x1234, 1)
    ...
    def test_the_part_decides_how_far_the_counter_reaches(self) -> None:
    ...
    def test_the_multiplicands_keep_their_sign(self) -> None:
        chip = oracle.prepared(0xFFFF0000, 1)

        self.assertEqual(chip.regs.k, reference.i16(chip.regs.k))

    def test_nothing_starts_clear(self) -> None:
        chip = oracle.prepared(0x99, 1)

        self.assertTrue(_sourced(chip.dataRAM).started(3))
        self.assertTrue(chip.programROM[5])


class SourcedTest(unittest.TestCase):
    def test_a_store_is_as_long_as_the_part_it_stands_for(self) -> None:
...
class AnswerTest(unittest.TestCase):
    def test_an_answer_is_the_width_a_state_has(self) -> None:
        self.assertEqual(len(oracle.answer(1, 1, 0)), instructions.WIDTH)

    def test_the_same_case_answers_the_same_way(self) -> None:
        self.assertEqual(oracle.answer(5, 0, 0x1234), oracle.answer(5, 0, 0x1234))

    def test_a_scratch_write_is_reported_with_its_address(self) -> None:
        seed, part = 7, 1
    ...
    def test_a_write_of_what_was_already_there_is_no_change_at_all(self) -> None:
        seed, part = 7, 1
        chip = oracle.prepared(seed, part)
        already = _sourced(chip.dataRAM).started(chip.regs.dp)

        found = oracle.answer(seed, part, 3 << 22 | already << 6 | 15)

        self.assertEqual(found[-16:-14], "00")

    def test_answering_many_cases_answers_one_state_each(self) -> None:
...
class AgainstRecordedTest(unittest.TestCase):
    def test_the_transliteration_reproduces_what_the_original_left_behind(self) -> None:
