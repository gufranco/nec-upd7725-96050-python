...
class CaseTest(unittest.TestCase):
    def test_a_case_names_a_seed_a_part_and_an_instruction(self):
    ...
    def test_every_source_and_destination_pair_is_reached(self):
        seen = set()
        for index in range(instructions.SETTLED):
            _, _, opcode = instructions.case_for(index)
            if opcode >> 22 == 0:
    ...
    def test_every_branch_the_part_has_is_reached(self):
        seen = set()
        for index in range(instructions.SETTLED):
            _, _, opcode = instructions.case_for(index)
            if opcode >> 22 == 2:
    ...
    def test_a_case_past_the_settled_ones_is_still_built(self):
...
class ReplayTest(unittest.TestCase):
    def test_replaying_a_case_answers_a_state(self):
...
class CorpusTest(unittest.TestCase):
    ...
    def test_every_recorded_state_is_the_width_a_state_has(self):
        found = instructions.load()

        self.assertEqual(len(instructions.expected_of(found, 0)), instructions.WIDTH)

    def test_the_corpus_records_as_many_states_as_it_claims_cases(self):
        found = instructions.load()
...
class ComparisonTest(unittest.TestCase):
    def test_two_identical_states_report_nothing(self):
        self.assertIsNone(instructions.disagreement("ab", "ab"))

    def test_a_state_that_differs_is_reported_with_both_sides(self):
        self.assertEqual(instructions.disagreement("ab", "ac"), ("ab", "ac"))


class AgainstCorpusTest(unittest.TestCase):
    def test_the_model_reproduces_every_state_the_reference_left(self):
        found = instructions.load()
...
class RecordTest(unittest.TestCase):
    def test_recording_asks_the_reference_for_every_case(self):
    ...
    def test_a_recorded_state_is_the_width_a_state_has(self):
        found = instructions.record(1)
...
class EncodingTest(unittest.TestCase):
    def test_a_state_survives_being_written_down_and_read_back(self):
