...
class MeasurementTest(unittest.TestCase):
    ...
    def _timed(self, seconds: Sequence[float]) -> speed.Timed:
        return speed.Timed("upd7725", instructions=1000, seconds=seconds)
    ...
    def test_the_rate_is_instructions_over_the_median(self) -> None:
    ...
    def test_the_share_of_real_time_is_the_rate_over_the_parts_own(self) -> None:
        found = speed.Timed("upd7725", instructions=speed.PART_HERTZ, seconds=[1.0])

        self.assertAlmostEqual(found.of_real_time, 1.0)
    ...
    def test_a_run_exactly_at_the_floor_beats_it(self) -> None:
        found = speed.Timed("upd7725", instructions=1000, seconds=[1.0])

        self.assertTrue(found.beats(1000))

    def test_a_measurement_prints_as_the_rate_it_found(self) -> None:
        self.assertIn("instructions per second", repr(self._timed([0.1])))
...
class RunTest(unittest.TestCase):
    ...
    def test_and_reports_the_part_it_ran(self) -> None:
        ...
        self.assertEqual(found.part, "upd7725")
...
class ReportTest(unittest.TestCase):
    def test_a_report_gives_the_rate_and_the_runtime(self) -> None:
        found = speed.Timed("upd7725", instructions=1000, seconds=[0.001])

        lines = " ".join(speed.lines_for(found, floor=100))

        self.assertIn("instructions per second", lines)
        self.assertIn("Python", lines)

    def test_and_the_fastest_and_slowest_run(self) -> None:
        found = speed.Timed("upd7725", instructions=1000, seconds=[0.001, 0.002, 0.003])
    ...
    def test_and_what_share_of_the_silicon_it_manages(self) -> None:
        found = speed.Timed("upd7725", instructions=1000, seconds=[0.001])

        self.assertIn("% of the", " ".join(speed.lines_for(found, floor=100)))

    def test_a_run_below_the_floor_says_so(self) -> None:
        found = speed.Timed("upd7725", instructions=1000, seconds=[10.0])
...
class EntryTest(unittest.TestCase):
    ...
    def test_a_run_that_beats_the_floor_passes(self) -> None:
        code, said = self._run(
            [],
            floor=100,
            run=lambda **_options: speed.Timed("upd7725", 1000, [0.001]),
        )
    ...
    def test_a_run_that_does_not_fails_and_says_so(self) -> None:
        code, said = self._run(
            [],
            floor=1_000_000,
            run=lambda **_options: speed.Timed("upd7725", 1000, [1.0]),
        )
