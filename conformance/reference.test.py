...
class WordTest(unittest.TestCase):
    def test_a_word_of_sixteen_bits_keeps_sixteen(self):
        self.assertEqual(reference.n16(0x1FFFF), 0xFFFF)

    def test_a_word_of_twenty_four_keeps_twenty_four(self):
        self.assertEqual(reference.n24(0x1FFFFFF), 0xFFFFFF)

    def test_a_signed_word_takes_the_sign_of_its_top_bit(self):
        self.assertEqual(reference.i16(0x8000), -0x8000)

    def test_and_leaves_a_small_one_alone(self):
        self.assertEqual(reference.i16(0x7FFF), 0x7FFF)


class RevisionTest(unittest.TestCase):
    def test_the_smaller_part_has_the_narrower_registers(self):
        found = reference.Upd96050(reference.UPD7725)

        self.assertEqual(found.regs.pc_mask, 0x7FF)

    def test_and_the_larger_one_reaches_further(self):
        found = reference.Upd96050(reference.UPD96050)

        self.assertEqual(found.regs.pc_mask, 0x3FFF)

    def test_a_revision_it_does_not_know_is_refused(self):
...
class ConsoleTest(unittest.TestCase):
    def test_a_wide_transfer_hands_the_low_half_back_first(self):
        found = reference.Upd96050(reference.UPD96050)
        found.regs.dr = 0xBEEF

        self.assertEqual(found.read_dr(), 0xEF)

    def test_and_the_high_half_second(self):
        found = reference.Upd96050(reference.UPD96050)
        found.regs.dr = 0xBEEF
        found.read_dr()
    ...
    def test_a_narrow_transfer_hands_back_one_half_and_stops_asking(self):
        found = reference.Upd96050(reference.UPD96050)
        found.regs.dr = 0xBEEF
        found.regs.sr.drc = 1
        found.regs.sr.rqm = 1
    ...
    def test_a_wide_write_takes_the_low_half_first(self):
        found = reference.Upd96050(reference.UPD96050)
    ...
    def test_and_the_high_half_of_a_write_second(self):
        found = reference.Upd96050(reference.UPD96050)
    ...
    def test_a_narrow_write_takes_one_half_and_stops_asking(self):
        found = reference.Upd96050(reference.UPD96050)
        found.regs.sr.drc = 1
        found.regs.sr.rqm = 1
    ...
    def test_the_status_the_console_reads_is_the_top_half_of_the_word(self):
        found = reference.Upd96050(reference.UPD96050)
        found.regs.sr.rqm = 1

        self.assertEqual(found.read_sr(), 0x80)

    def test_writing_the_status_from_outside_does_nothing(self):
        found = reference.Upd96050(reference.UPD96050)
    ...
    def test_the_console_reads_the_low_half_of_a_scratch_word_from_the_even_address(self):
        found = reference.Upd96050(reference.UPD96050)
        found.dataRAM[0] = 0xBEEF

        self.assertEqual(found.read_dp(0), 0xEF)

    def test_and_the_high_half_from_the_odd_one_beside_it(self):
        found = reference.Upd96050(reference.UPD96050)
        found.dataRAM[0] = 0xBEEF

        self.assertEqual(found.read_dp(1), 0xBE)

    def test_writing_a_half_leaves_the_other_alone(self):
        found = reference.Upd96050(reference.UPD96050)
        found.dataRAM[0] = 0xBEEF
...
class ExecutionTest(unittest.TestCase):
    def test_the_counter_moves_on_by_one(self):
        found = reference.Upd96050(reference.UPD96050)
    ...
    def test_every_form_is_reachable(self):
        for form in range(4):
            found = reference.Upd96050(reference.UPD96050)
            found.regs.sp = 1
    ...
    def test_every_source_and_every_destination_is_reachable(self):
        for src in range(16):
            for dst in range(16):
                found = reference.Upd96050(reference.UPD96050)
    ...
    def test_every_pointer_step_is_reachable(self):
        for dpl in range(4):
