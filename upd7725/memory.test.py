...
class ShapeTest(unittest.TestCase):
    def test_each_store_is_as_long_as_it_was_asked_to_be(self):
    ...
    def test_a_store_starts_holding_what_it_was_told_to_hold(self):
        found = memory.Stores(program_words=4, table_words=4, scratch_words=4, fill=0xAB)

        self.assertEqual(found.scratch[0], 0xAB)

    def test_and_starts_clear_when_that_is_what_was_asked(self):
...
class WidthTest(unittest.TestCase):
    def test_a_program_word_is_twenty_four_bits_wide(self):
    ...
    def test_a_table_word_is_sixteen(self):
        found = memory.Stores(program_words=4, table_words=4, scratch_words=4)
    ...
    def test_and_so_is_a_scratch_word(self):
        found = memory.Stores(program_words=4, table_words=4, scratch_words=4)
    ...
    def test_a_negative_word_is_kept_as_the_bits_it_stands_for(self):
        found = memory.Stores(program_words=4, table_words=4, scratch_words=4)
...
class AddressTest(unittest.TestCase):
    def test_an_address_past_the_end_comes_back_to_the_start(self):
        found = memory.Stores(program_words=4, table_words=4, scratch_words=4)
    ...
    def test_and_reads_from_there_too(self):
        found = memory.Stores(program_words=4, table_words=4, scratch_words=4)
        found.scratch[1] = 0x4321

        self.assertEqual(found.scratch[5], 0x4321)

    def test_a_store_says_how_long_it_is(self):
...
class ByteAccessTest(unittest.TestCase):
    def test_the_console_reads_the_low_half_of_a_scratch_word_first(self):
        found = memory.Stores(program_words=4, table_words=4, scratch_words=4)
        found.scratch[0] = 0xBEEF

        self.assertEqual(found.read_byte(0), 0xEF)

    def test_and_the_high_half_from_the_odd_address_beside_it(self):
        found = memory.Stores(program_words=4, table_words=4, scratch_words=4)
        found.scratch[0] = 0xBEEF

        self.assertEqual(found.read_byte(1), 0xBE)

    def test_writing_a_byte_leaves_the_other_half_alone(self):
        found = memory.Stores(program_words=4, table_words=4, scratch_words=4)
        found.scratch[0] = 0xBEEF
...
class SourceTest(unittest.TestCase):
    def test_a_word_nothing_wrote_comes_from_the_source(self):
        found = memory.Store(4, 16, source=lambda at: at * 0x1111)

        self.assertEqual(found[2], 0x2222)

    def test_a_source_word_wider_than_the_store_is_cut_to_fit(self):
        found = memory.Store(4, 16, source=lambda at: 0x1FFFF)

        self.assertEqual(found[0], 0xFFFF)

    def test_a_word_that_was_written_comes_back_instead(self):
        found = memory.Store(4, 16, source=lambda at: at)
        found[2] = 0xABCD

        self.assertEqual(found[2], 0xABCD)

    def test_a_store_names_the_words_that_are_not_what_it_started_with(self):
        found = memory.Store(4, 16, source=lambda at: at)
        found[2] = 0xABCD

        self.assertEqual(found.changed(), {2: 0xABCD})

    def test_writing_a_word_back_the_way_it_was_changes_nothing(self):
        found = memory.Store(4, 16, source=lambda at: at)
        found[2] = 2

        self.assertEqual(found.changed(), {})

    def test_the_same_holds_for_a_store_that_was_filled_rather_than_sourced(self):
    ...
    def test_a_sourced_store_can_be_handed_to_the_three_together(self):
        found = memory.Stores(
            program_words=4,
            table_words=4,
            scratch_words=4,
...
class LoadTest(unittest.TestCase):
    def test_a_program_arrives_three_bytes_to_a_word(self):
    ...
    def test_a_table_arrives_two_bytes_to_a_word(self):
    ...
    def test_a_program_longer_than_the_store_is_refused_rather_than_trimmed(self):
    ...
    def test_and_so_is_a_table(self):
        found = memory.Stores(program_words=1, table_words=1, scratch_words=1)

        with self.assertRaises(memory.TooLarge):
            found.load_table(bytes(4))

    def test_a_program_that_does_not_divide_into_words_is_refused(self):
        found = memory.Stores(program_words=4, table_words=4, scratch_words=4)

        with self.assertRaises(memory.NotWholeWords):
            found.load_program(bytes(4))

    def test_and_a_table_that_does_not_divide_into_words_either(self):
        found = memory.Stores(program_words=4, table_words=4, scratch_words=4)

        with self.assertRaises(memory.NotWholeWords):
            found.load_table(bytes(3))

    def test_a_short_program_leaves_the_rest_of_the_store_as_it_was(self):
    ...
    def test_a_fill_wider_than_the_store_keeps_only_the_bits_that_fit(self):
